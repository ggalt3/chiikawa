...
NAMESPACE_BEGIN(Vars)
	...
	NAMESPACE_BEGIN(Misc)
		...
		NAMESPACE_BEGIN(PixelFinder, PixelFinder)
			CVar(Enabled, "Enabled", false);
			CVar(DrawLineKey, "Draw line key", 0x52);
			CVar(SavePointKey, "Save point key", 0x45);
			CVar(ClearPointsKey, "Clear points key", 0x58);
			CVar(DetectionStep, "Detection step", 2.0f, SLIDER_MIN, 1.0f, 10.0f, 0.5f);
			CVar(PointColor, VA_LIST("Point color", "PixelFinder point color"), Color_t(0, 255, 0, 255), VISUAL);
			CVar(SavedPointColor, VA_LIST("Saved point color", "PixelFinder saved point color"), Color_t(255, 165, 0, 255), VISUAL);
			CVar(LineColor, VA_LIST("Line color", "PixelFinder line color"), Color_t(200, 200, 200, 255), VISUAL);
		NAMESPACE_END(PixelFinder)

		NAMESPACE_BEGIN(Sound, Sound)
			CVarEnum(Block, "Block", 0b0, DROPDOWN_MULTI, nullptr,
				VA_LIST("Voice", "Footsteps", "Weapon sounds", "Ambient", "Noisemaker", "FryingPan", "Water"),
				Voice = 1 << 0, Footsteps = 1 << 1, WeaponSounds = 1 << 2, Ambient = 1 << 3, Noisemaker = 1 << 4, FryingPan = 1 << 5, Water = 1 << 6);
